...
export class DefinirPeriodeCouranteUseCase {
  ...
  async definirPeriode(periodeId: string): Promise<void> {
    const periode = await this.anneeRepository.findPeriodeById(periodeId);
  ...
  async definirSequence(sequenceId: string): Promise<void> {
    const sequence = await this.anneeRepository.findSequenceById(sequenceId);
