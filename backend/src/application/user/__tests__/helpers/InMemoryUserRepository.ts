...
export class InMemoryUserRepository implements UserRepository {
  ...
  async findByPhone(_phone: string, _schoolId: string): Promise<User | null> {
    return null;
  }
  ...
  async findByRole(schoolId: string, role: UserRole): Promise<User[]> {
    return [...this.store.values()].filter(
      u => u.schoolId === schoolId && u.role === role
    );
  }
  ...
  async findByIdWithRefreshVersion(
    ...
    if (!user) return null;
    return { user, refreshTokenVersion: 0 };
  }
  ...
  async listerRolesAvecMotDePasse(email: string, schoolId: string, _plainPassword: string): Promise<string[]> {
    return [...this.store.values()].filter(u => u.email === email && u.schoolId === schoolId && u.isActive).map(u => u.role);
  }
  ...
  async mettreAJourAvecProfil(_userId: string, _data: any): Promise<void> {}
