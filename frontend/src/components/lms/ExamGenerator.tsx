...
const ExamGenerator = ({ open, onOpenChange, onSuccess }: Props) => {
  ...
  const startPolling = (generationId: string) => {
    ...
    const poll = async () => {
      try {
        const { data } = await api.get(`/exams/generation/${generationId}`);
        ...
        if (generation.status === "failed") {
          ...
          toast.error(generation.message || "Exam generation failed");
        }
      } catch (error: any) {
        if (pollingRef.current) {
          window.clearInterval(pollingRef.current);
  ...
  const onSubmit = async (values: FormValues) => {
    try {
      ...
      setGenerationMessage(data.message || "Exam generation started");
      setGenerationProgress(20);
      toast.success("AI is generating the exam.");
            ...
            {generationStatus !== "idle" && (
              <div className="space-y-2 rounded-md border p-3">
                <div className="flex items-center justify-between text-xs">
                  <span className="font-medium capitalize">{generationStatus}</span>
                  <span className="text-muted-foreground">
                    {generationMessage || "Preparing exam generation..."}
                  </span>
                </div>
