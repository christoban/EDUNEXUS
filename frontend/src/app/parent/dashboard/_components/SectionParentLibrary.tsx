    ...
    <div style={{ padding: '28px 32px', overflowY: 'auto', height: '100%' }}>
      ...
      {!loading && !error && loans.length > 0 && (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16, marginBottom: 24 }}>
          {[
            { icon: '📚', bg: 'var(--blue-light)', val: loans.length, label: t('library.totalLoans'), color: 'var(--blue)' },
            { icon: '📖', bg: 'var(--green-light)', val: active,        label: t('library.active'),    color: 'var(--green)' },
            { icon: '⏰', bg: 'var(--red-light)', val: overdue,       label: t('library.overdue'),   color: 'var(--red)' },
          ].map((k, i) => (
                  ...
                  <tr key={l.id}
                    ...
                    <td style={tdSt}>
                      <span style={{ padding: '4px 10px', borderRadius: 20, fontSize: 13, fontWeight: 800, background: isOverdue ? 'var(--red-light)' : badge.bg, color: isOverdue ? 'var(--red)' : badge.color }}>
                        {isOverdue ? t('library.overdueBadge') : badge.label}
                      </span>
                    </td>
                  </tr>
                )
